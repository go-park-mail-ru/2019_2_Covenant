...
func (ar *AlbumRepository) AddTrack(albumID uint64, name string) (uint64, error) {
	...
	var id uint64
	err := ar.db.QueryRow("INSERT INTO tracks (album_id, name) VALUES ($1, $2) RETURNING id", albumID, name).Scan(&id)
	if err != nil {
		return 0, err
	}
